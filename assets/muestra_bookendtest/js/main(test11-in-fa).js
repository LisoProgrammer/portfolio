...
function time() {
  ...
  if (timeS < 31 && timeM === 0) {
    ...
    cAds.innerHTML =
      "<p>Se acaba el tiempo, ¡Te quedan menos de 30 segundos!</p>";
  }
...
function verificar() {
  ...
  const cuestion = ["No copy", 2, 1, 3, 1, 2, 4, 3, 3, 1, 2];
  ...
  for (let i = 1; i < cuestion.length; i++) {
    ...
    if (e.length == 0) {
      document.getElementsByClassName("reco" + i)[0].className =
        "recoBorderInf reco" + i;
      document.getElementsByClassName("reco" + i)[0].innerHTML =
        "No haz seleccionado respuesta a este inciso.";
      ...
      while (cont < cuestion.length) {
        ...
        if (e == cuestion[i]) {
          ...
          for (let j = 0; j < 4; j++) {
            document
              .getElementById("mod" + i)
              .r[j].setAttribute("disabled", "disabled");
          }
        } else {
          document.getElementsByClassName("reco" + i)[0].className =
            "reco" + i + " recoBorder";
  ...
  if (punt < 50) {
    ene.innerHTML =
      "Obtuviste " +
      punt +
      " puntos de 100 <br> Practica un poco más, tu puedes lograrlo.";
  ...
  if (punt <= 70 && punt >= 50) {
    ene.innerHTML =
      "Obtuviste " +
      punt +
      " puntos de 100 <br> Sigue practicando, estás muy cerca a la meta. Te faltan " +
      (100 - punt) +
      " puntos";
    ene.appendChild(newButton);
    inputHiddenPuntos.value = punt;
  ...
  if (punt > 70) {
    ...
    ene.innerHTML =
      "Obtuviste " +
      punt +
      " puntos de 100 <br> Genial, eres un gran estudiante.";
    ene.appendChild(newButton);
    inputHiddenPuntos.value = punt;
    getValues();
